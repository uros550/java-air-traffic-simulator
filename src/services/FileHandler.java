...
public class FileHandler {
    ...
    public void importAirports(String filename) throws IOException, AirportException, FileException {
    	//check file
        ...
        if (filename.toLowerCase().endsWith(".json")) {
            importAirportsJSON(file);
        } 
        else if (filename.toLowerCase().endsWith(".csv")){
            importAirportsCSV(file);
        }
        else {
    ...
    public void importFlights(String filename) throws IOException, FlightException, FileException {
    	//check file
        File file = new File(filename);
        if (!file.exists()) {
            throw new IOException("File " + filename + " doesn't exist!");
        ...
        //check format
        if (filename.toLowerCase().endsWith(".json")) {
            importFlightsJSON(file);
        } 
        else if (filename.toLowerCase().endsWith(".csv")){
            importFlightsCSV(file);
        }
        else {
        	throw new FileException("File " + filename + " isn't JSON or CSV format!");
        }
    ...
    //CSV format
    private void importAirportsCSV(File file) throws IOException, AirportException {
    ...
    private void importFlightsCSV(File file) throws IOException, FlightException {
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(file), StandardCharsets.UTF_8))) {
            String line;
            //check header
            boolean isHeader = true;
            ...
            while ((line = reader.readLine()) != null) {
                ...
                String[] parts = line.split(",");
                if (parts.length != 4) {
                    throw new FlightException("Expected 4 columns!");
                }
    ...
    //JSON format
    private void importAirportsJSON(File file) throws IOException, AirportException {
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(file), StandardCharsets.UTF_8))) {
            String line;
            while ((line = reader.readLine()) != null) {
            	//remove leading and trailing white spaces
    ...
    private void importFlightsJSON(File file) throws IOException, FlightException {
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(file), StandardCharsets.UTF_8))) {
            String line;
            while ((line = reader.readLine()) != null) {
            	//remove leading and trailing white spaces
